...
/**
 * LE_PARA_AVL (PTAVL)
 * Retorna AVL contendo as palavras do arquivo lido, computando o número de comparações necessária para tal.
 *
 * FILE *entrada: ponteiro para o arquivo que será lido;
...
PtAVL* le_para_avl(FILE *entrada, int *comp_ger, int *rot, int *ok)
{
    ...
    while (fgets(linha,LINHAS,entrada)) //percorre todo o arquivo lendo linha a linha
    {
        palavra = strtok(linha, "\n"); //tokeniza usando como delimitador o espaço em branco
        while (palavra != NULL) //enquanto ler alguma palavra válida
        {
            avl = insere_avl(avl, palavra, comp_ger, rot, ok); //insere palavra a palavra na AVL, chamando a função insere_avl
            palavra = strtok (NULL, "\n"); //tokeniza usando como delimitador o espaço em branco
        }
    }
    fclose(entrada);
