...
int main(int argc, char *argv[])
{
    ...
    PtAVL *avl0, *avl1;                                                  //avl0 e avl1 recebem árvores binárias de pesquisa balanceadas (AVL)
    ...
    else
    {
        ...
        caixa_baixa_avl(entrada, &avl1, &comp_ger, &rot, &ok);            //converte argv[1] para caixa baixa e exporta o resultado para temp.txt
        ...
        nodos = conta_avl(avl1, &comp_rel);                                    //calcula número de nodos da AVL
        altura = altura_avl(avl1, &comp_rel);                                  //calcula altura da AVL
        fb = fator_balanceamento_avl(avl1, &comp_rel);                         //calcula FB da AVL
        le_operacoes(op, resultado, avl0, avl1, &comp_rel, &rot_rel, &ok_rel); //interpreta e realiza as operações solicitadas
        ...
        //gera relatório
        relatorio_avl(saida, resultado, nodos, altura, fb, miliseconds_ger, miliseconds_rel, comp_ger, comp_rel, rot, rot_rel);
        return 0;
    }
}
